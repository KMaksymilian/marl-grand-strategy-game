...
impl Building {
    ...
    pub fn calculate_demand(&self) -> HashMap<ResourceType, u32> {
        ...
        let labor_needed = (self.definition.max_workers as f32 * self.target_efficiency).floor() as u32;
        if labor_needed > 0 {
        ...
        for behavior in &self.definition.behaviors {
            if let BuildingBehavior::Production { inputs, .. } = behavior {
                for (res, amount) in inputs {
                    *demand.entry(*res).or_insert(0) += (*amount as f32 * self.target_efficiency).floor() as u32;
                }
            }
        }
        demand
...
mod tests {
    ...
    fn test_resolve_allocation_no_workers_no_production() {
        ...
        assert_eq!(
            *result.production.unused.get(&ResourceType::Wood).unwrap_or(&0),
            50,
    ...
    fn test_resolve_allocation_inactive_building() {
        ...
        assert_eq!(building.current_workers, 0, "Inactive building should reject workers.");
    }
    ...
    fn test_resolve_allocation_full_efficiency() {
        ...
        assert_eq!(
            *result.production.produced.get(&ResourceType::Stone).unwrap_or(&0),
            20
        );
        assert_eq!(
            *result.production.unused.get(&ResourceType::Wood).unwrap_or(&0),
    ...
    fn test_resolve_allocation_fractional_production_with_bottleneck() {
        ...
        assert_eq!(
            *result.production.unused.get(&ResourceType::Grain).unwrap_or(&0),
            0
        );
